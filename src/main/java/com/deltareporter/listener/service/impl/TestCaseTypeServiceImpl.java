...
public class TestCaseTypeServiceImpl
  implements TestCaseTypeService
{
  ...
  public TestCaseType registerTestCase(String name, String datetime, Integer test_suite_id, Integer test_run_id) {
    return this.deltaClient.registerTestCase(name, datetime, test_suite_id, test_run_id);
  }
  ...
  public TestCaseType finishTest(TestCaseType test) {
    ...
    if (result.getStatus() != 200 && result.getObject() == null) {
      throw new RuntimeException("Unable to register test " + test.getName() + " for delta service: " + this.deltaClient.getServiceUrl());
    }
