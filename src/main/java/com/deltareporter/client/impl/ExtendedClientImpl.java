  ...
  implements ExtendedClient
{
  ...
  public Integer registerLaunch(String name, String project) {
    ...
    LOGGER.debug("Launch details to create:" + String.format(launchDetails, new Object[] { name, project }));

    HttpClient.Response<LaunchType> response = this.client.createLaunch(launch);
    Integer launch_id = (int)response.getObject().getId();

    if (launch_id == null) {
      throw new RuntimeException("Unable to create launch: '" + name + "' for delta service: " + this.client.getServiceUrl());
    }
  ...
  public Integer registerTestRun(String test_type, Integer launch_id, String datetime) {
    ...
    LOGGER.debug("Test Run details to create:" + String.format(testRunDetails, new Object[] { test_type, launch_id, datetime }));
    ...
    if (test_run_id == null) {
      throw new RuntimeException("Unable to create test run for delta service: " + this.client.getServiceUrl());
    }
  ...
  public TestSuiteHistoryType registerTestSuiteHistory(String name, String test_type, String start_datetime, Integer test_run_id) {
    TestSuiteHistoryType test_suite_history = new TestSuiteHistoryType(name, test_type, start_datetime, test_run_id);
    String suiteHistoryDetails = "Name: %s, Test Type: %s, DateTime: %s, TestRunId: %s";
    LOGGER.debug("Test Suite History details to create:" + String.format(suiteHistoryDetails, new Object[] { name, test_type, start_datetime, test_run_id }));

    HttpClient.Response<TestSuiteHistoryType> response = this.client.createTestSuiteHistory(test_suite_history);
    test_suite_history = (TestSuiteHistoryType)response.getObject();

    if (test_suite_history == null) {
      throw new RuntimeException("Unable to create test suite history for delta service: " + this.client.getServiceUrl());
    }
    LOGGER.debug("Test Suite History created! Test Suite History ID:" + test_suite_history.getTest_suite_history_id());
  ...
  public void finishTestSuiteHistory(Integer test_suite_history_id, String end_datetime, String test_suite_status) {
    TestSuiteHistoryType test_suite_history = new TestSuiteHistoryType(test_suite_history_id, end_datetime, test_suite_status);
    String suiteHistoryDetails = "ID: %d, End Datetime: %s, Test Suite Status: %s";
    LOGGER.debug("Test Suite History details to update:" + String.format(suiteHistoryDetails, new Object[] { test_suite_history_id, end_datetime, test_suite_status }));

    HttpClient.Response<TestSuiteHistoryType> response = this.client.finishTestSuiteHistory(test_suite_history);
  }
  ...
  public void finishTestRun(Integer test_run_id, String end_datetime, String test_run_status) {
    ...
    LOGGER.debug("Test Run details:" + String.format(testRunDetails, new Object[] { test_run_id, end_datetime, test_run_status }));
  ...
  public TestCaseType registerTestCase(String name, String datetime, Integer test_suite_id, Integer test_run_id) {
    ...
    LOGGER.debug("Test Case details for registration:" +
        String.format(testCaseDetails, new Object[] { name, datetime, test_suite_id, test_run_id }));
    ...
    if (testCase == null) {
      throw new RuntimeException("Unable to register test case '" +
          String.format(testCaseDetails, new Object[] { name, datetime, test_suite_id, test_run_id }) + "' for delta service: " + this.client.getServiceUrl());
    }
